...
class Encoder(torch.nn.Module):
    def __init__(self, wordsize, embaddingsize, hiddensize, numlayers, isbidirectional):
        ...
        self.conv1 = torch.nn.Conv1d(1, 16, 3, 1, padding=1)
        self.conv2 = torch.nn.Conv1d(16, 16, 3, 1, padding=1)
        self.conv3 = torch.nn.Conv1d(16, 1, 3, 1, padding=1)
    ...
    def forward(self, x, h_state):
        ...
        embadc = F.relu(self.conv1(embad))
        embadc = F.relu(self.conv2(embadc))
        embad = F.relu(self.conv3(embadc) + embad)
...
class Attention_Decoder(torch.nn.Module):
    def __init__(self, wordsize, embaddingsize, hiddensize, numlayers,
        ...
        if isbidirectional:
            self.attention = torch.nn.Linear(embaddingsize + hiddensize * numlayers * 2, encoder_outs_maxlength)
            ...
            self.attention = torch.nn.Linear(embaddingsize + hiddensize * numlayers, encoder_outs_maxlength)
    ...
    def forward(self, x, h_state, encoder_outs):
        ...
        pre_concat = torch.cat((h_state.view(1, -1), embadded[0]), dim=1)
        attention_weight = F.softmax(self.attention(pre_concat), dim=1)
        attention_app = torch.bmm(attention_weight.unsqueeze(0), encoder_outs.unsqueeze(0))
        attention_com = F.relu(
            self.attention_combine(torch.cat((embadded[0], attention_app[0]), dim=1))).unsqueeze(0)
...
if isTrain:
    ...
    if isLoad:
        encoder.load_state_dict(torch.load('./model/translate_attention_encoder4.pkl'))
        decoder.load_state_dict(torch.load('./model/translate_attention_decoder4.pkl'))
    encoder.train()
    decoder.train()
    if withCuda:
        encoder.cuda()
    ...
    for iter in range(1000000):
        ...
        if (iter + 1) %printevery == 0:
            ...
            loss_avg = 0
            torch.save(encoder.state_dict(), './model/translate_attention_encoder4.pkl')
            torch.save(decoder.state_dict(), './model/translate_attention_decoder4.pkl')
            print('Save Model')
# else:
