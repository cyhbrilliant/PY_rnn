...
if isTrain:
    ...
    if isLoad:
        encoder.load_state_dict(torch.load('./model/translate_attention_encoder3.pkl'))
        decoder.load_state_dict(torch.load('./model/translate_attention_decoder3.pkl'))
    encoder.train()
    decoder.train()
    if withCuda:
        encoder.cuda()
    ...
    for iter in range(1000000):
        ...
        if (iter + 1) %printevery == 0:
            ...
            loss_avg = 0
            torch.save(encoder.state_dict(), './model/translate_attention_encoder3.pkl')
            torch.save(decoder.state_dict(), './model/translate_attention_decoder3.pkl')
            print('Save Model')
# else:
